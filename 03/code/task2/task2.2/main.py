...
trainset = ImageWoof(root=root_path, train=True, transform=transform, img_size=160)

testset = ImageWoof(root=root_path, train=False, transform=transform, img_size=160)
...
def validate(model):
    ...
    with tqdm(val_loader, unit="batch") as tepoch:
        ...
        log = ' val loss: {:.4f} accuracy: {:.4f} best_loss: {:.4f}\n'.format(val_loss, 100.*correct/total, best_loss)
        print(log)
        with open(path + "/log.txt", 'a') as file:
            file.write(log)    
    return val_loss    
...
if __name__ == "__main__":
    ...
    elif(params.opt == "sparseadam"):
        optimizer = optim.SparseAdam(model.parameters(), lr=params.lr)
    else:
        optimizer = optim.SGD(model.parameters(), lr=params.lr)

    #scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.05)
    #scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.05)
    ...
    for epoch in range(0, epochs):
        ...
        train(model, epoch)
        validate(model) #val_loss = validate(model)
